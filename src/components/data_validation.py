...
class DataValidation:
    ...
    def initiate_data_validation(self)-> DataValidationArtifact:
        try:
            ...
            status=self.is_colmn_exist(df=train_df)
            if not status:
                validation_error_msg+=f"Columns are missing in training dataframe."
            else:
                logging.info(f"All catagorical/int columns present in training dataframe: {status}")
                
            
            status=self.is_colmn_exist(df=test_df)
            if not status:
                validation_error_msg+=f"Columns are missing in test dataframe."
            else:
                logging.info(f"All catagorical/int columns present in test dataframe: {status}")
